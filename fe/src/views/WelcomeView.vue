...
import { createRoom, joinRoom, login, type LoginUser } from '../api/prototypeApi'
import { setCurrentUser } from '../composables/useCurrentUser'

type Step = 'login' | 'mode' | 'create' | 'created' | 'join'
...
// 로그인 후 반환된 유저 정보 임시 저장 (roomId 없는 상태)
...
async function handleLogin() {
  ...
  try {
    ...
    if (user.roomId && response.room) {
      // 기존 방이 있으면 바로 로그인
      setCurrentUser({ id: user.id, name: user.name, roomId: user.roomId })
    ...
    errorMessage.value = '로그인에 실패했습니다. 다시 시도해 주세요.'
  } finally {
...
async function handleCreateRoom() {
  ...
  try {
    const response = await createRoom(
      roomName.value.trim(),
      pendingUser.value.id,
      pendingUser.value.name,
    )
...
function finishCreate() {
  if (!createdRoom.value || !pendingUser.value) return
  setCurrentUser({
    id: pendingUser.value.id,
    name: pendingUser.value.name,
    roomId: createdRoom.value.id,
  })
...
<template>
  <div class="welcome-shell">
    <div class="welcome-card">
      ...
      <!-- Step 1: 이름으로 로그인 -->
      <form v-if="step === 'login'" class="welcome-form" @submit.prevent="handleLogin">
        ...
        <p class="welcome-hint">처음이라면 계정이 자동 생성됩니다.</p>
      ...
      <div v-else-if="step === 'mode'" class="welcome-mode">
        <p class="mode-greeting">안녕하세요, <strong>{{ pendingUser?.name }}</strong> 님!</p>
